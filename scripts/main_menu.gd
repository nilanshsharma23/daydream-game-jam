...
func _on_play_button_pressed() -> void:
	...
	var tween: Tween = create_tween()
	tween.tween_property(label, "position", Vector2(0, -186), 0.25).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
	for i in buttons.get_children():
		tween.tween_property(i, "position", Vector2(-250, i.position.y), 0.125).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
	tween.tween_callback(SceneSwitcher.switch_scene.bind(Scenes.HOW_TO_PLAY_SCENE))
